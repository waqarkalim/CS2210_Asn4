...
public class TestBST {
	...
	public static void main(String[] args) {
		...
		try {
			...
			else System.out.println("Test 4 failed cuz res is not null -------------------------------------");
		}
		catch(Exception e) {
		...
		try {
			...
			System.out.println("Test 5 failed cuz exception was not caught -------------------------------------");
		}
		catch(InexistentKeyException e) {
			System.out.println("Test 5 passed -------------------------------------");
		}
		catch (Exception e) {
			System.out.println("Test 5 failed cuz the wrong exception was caught -------------------------------------");
		}
		...
		try {
			...
			tree.put(tree.getRoot(),pixels[1]);
			System.out.println("Node1 put");
			tree.put(tree.getRoot(),pixels[0]);
			System.out.println("Node2 put");
			for (int i = 2; i < 5; ++i)
				tree.put(tree.getRoot(),pixels[i]);
			System.out.println("All of the pixels are added");
			res = tree.successor(tree.getRoot(),points[3]);
			System.out.println("Successor method returns something " + res);
			System.out.println("point[1] is (" + points[1].xCoord() + ", " + points[1].yCoord() + ")");
			System.out.println("The compareTo value of (" + res.getLocation().xCoord() + ", " + res.getLocation().yCoord()  +  ") is " + res.getLocation().compareTo(points[1]));
			if (res.getLocation().compareTo(points[1]) == 0)
				System.out.println("Test 6 passed -------------------------------------");
			else System.out.println("Test 6 failed cuz compareTo value is not 0 -------------------------------------");
		}
		catch (Exception e) {
			System.out.println("Test 6 failed cuz of exception -------------------------------------");
		}
		...
		try {
			tree.remove(tree.getRoot(),p);
			res = tree.successor(tree.getRoot(),new Location(1,25));
